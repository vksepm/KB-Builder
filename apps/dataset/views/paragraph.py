...
class Paragraph(APIView):
    ...
    class Download(APIView):
        ...
        @action(methods=['GET'], detail=False)
        @swagger_auto_schema(operation_summary="下载段落",
                             operation_id="下载段落",
                             manual_parameters=ParagraphSerializers.DownloadSerializer.get_request_params_api(),
                             responses={200: '下载文件'},  # 更新 responses
                             tags=["知识库/文档/段落"])
        @has_permissions(
            lambda r, k: Permission(group=Group.DATASET, operate=Operate.USE,
                                    dynamic_tag=k.get('dataset_id')))
