<template>
  <LayoutContainer :header="documentDetail?.name" back-to="-1" class="document-detail">
    <template #header>
      <el-text type="info" v-if="documentDetail?.type === '1'">（文档地址：
      ...
      <div class="document-detail__header">
        <el-button @click="downloadParagraphs" :disabled="loading" v-if="isBatch === false">
          下载所有内容
        </el-button>
    ...
    <div class="document-detail__main p-16"
      ...
      <div class="flex-between p-8">
        ...
        <el-input v-model="search" placeholder="搜索" class="input-with-select" style="width: 260px"
          @change="searchHandle" clearable>
          <template #prepend>
      ...
      <el-scrollbar>
        <div class="document-detail-height">
          ...
          <InfiniteScroll v-else :size="paragraphDetail.length" :total="paginationConfig.total"
            ...
            <el-row>
              <el-col :xs="24" :sm="12" :md="8" :lg="6" :xl="6" v-for="(item, index) in paragraphDetail" :key="index"
                class="p-8">
                <!-- 批量操作card -->
                <CardBox v-if="isBatch === true" shadow="hover" :title="item.title || '&nbsp'"
                  :description="item.content" class="document-card cursor"
                  :class="multipleSelection.includes(item.id) ? 'selected' : ''" :showIcon="false"
                ...
                <!-- 非批量操作card -->
                <CardBox v-else shadow="hover" :title="item.title || '&nbsp'" :description="item.content"
                  class="document-card cursor" :class="item.is_active ? '' : 'disabled'" :showIcon="false"
                  @click="editParagraph(item)">
                  <div class="active-button" @click.stop>
                    <el-switch v-model="item.is_active" @change="changeState($event, item)" size="small" />
                  </div>
                        ...
                        <el-dropdown trigger="click">
                          ...
                          <template #dropdown>
                            <el-dropdown-menu>
                              ...
                              <!--                                迁移</el-dropdown-item-->
                              <!--                              >-->
                              <el-dropdown-item icon="Delete" @click.stop="deleteParagraph(item)">删除</el-dropdown-item>
                              <el-dropdown-item icon="Download"
                                @click.stop="downloadParagraph(index)">下载</el-dropdown-item>
                            </el-dropdown-menu>
                          </template>
                        </el-dropdown>
                      </span>
      ...
      <div class="mul-operation border-t w-full" v-if="isBatch === true">
        ...
        <!--          迁移-->
        <!--        </el-button>-->
        <el-button @click="downloaSelectedParagraphs" :disabled="multipleSelection.length === 0">
          下载所选内容
        </el-button>
...
function downloaSelectedParagraphs() {
  const tempArr = paragraphDetail.value.map((item, index) => {
    return { ...item, index }
  })
  tempArr.filter(
    (v) => multipleSelection.value.includes(v.id)
    ...
    link.href = url
    const lastDotIndex = documentDetail.value.name.lastIndexOf('.');
    const basename = lastDotIndex === -1
      ? documentDetail.value.name
      : documentDetail.value.name.substring(0, lastDotIndex);
    const filename = basename + '-' + item.index + '.txt';
...
function downloadParagraph(index: any) {
  const { content } = paragraphDetail.value[index]
  ...
  const basename = lastDotIndex === -1
    ? documentDetail.value.name
    : documentDetail.value.name.substring(0, lastDotIndex);
  const filename = basename + '-' + index + '.txt';
  link.setAttribute('download', filename);
  document.body.appendChild(link)
  link.click()
  document.body.removeChild(link)
