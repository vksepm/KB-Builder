                ...
                <el-dropdown trigger="click" @command="dropdownHandle">
                  ...
                  <template #dropdown>
                    <el-dropdown-menu style="width: 100px">
                      <el-dropdown-item :class="filterMethod ? '' : 'is-active'" command=""
                        class="justify-center">全部</el-dropdown-item>
                      <template v-for="(value, key) of hitHandlingMethod" :key="key">
                        <el-dropdown-item :class="filterMethod === key ? 'is-active' : ''" class="justify-center"
                          :command="key">{{ value }}</el-dropdown-item>
                      </template>
                    </el-dropdown-menu>
                  </template>
                </el-dropdown>
...
function syncDataset() {
  SyncWebDialogRef.value.open(child_id.value)
}
...
function rowClickHandle(row: any, column: any) {
  ...
  router.push({ path: `/dataset/${child_id.value}/${row.id}` })
}
...
function creatQuickHandle(val: string) {
  ...
  const obj = [{ name: val }]
  useStore().document
    .asyncPostDocument(child_id.value, obj)
    .then(() => {
      getList()
      MsgSuccess('创建成功')
    })
...
function syncMulDocument() {
  ...
  documentApi.delMulSyncDocument(child_id.value, arr, loading).then(() => {
    MsgSuccess('同步文档成功')
    getList()
  })
}
...
function deleteMulDocument() {
  ...
  documentApi.delMulDocument(child_id.value, arr, loading).then(() => {
    MsgSuccess('批量删除成功')
...
function updateData(documentId: string, data: any, msg: string) {
  documentApi.putDocument(child_id.value, documentId, data, loading).then((res) => {
...
function getList(bool?: boolean) {
  ...
  documentApi
    .getDocument(child_id.value, paginationConfig.value, param, bool ? undefined : loading)
    .then((res) => {
...
async function getDetail() {
  await dataset.asyncGetDatasetDetail(id, loading).then((res: any) => {
    ...
    child_id.value = res.data.child_id
  })
}
